...
export interface ClientOptions {
  ...
  /**
   * Defaults to process.env['CHARTEST_PASSWORD'].
   */
   ...
   *
   * Defaults to process.env['CHARTEST_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['CHARTEST_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Chartest API.
 */
export class Chartest {
  username: string;
  password: string;
  apiKey: string;
   ...
   */
  constructor({
    baseURL = readEnv('CHARTEST_BASE_URL'),
    username = readEnv('CHARTEST_USERNAME'),
    password = readEnv('CHARTEST_PASSWORD'),
    apiKey = readEnv('CHARTEST_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (username === undefined) {
      throw new Errors.ChartestError(
        "The CHARTEST_USERNAME environment variable is missing or empty; either provide it, or instantiate the Chartest client with an username option, like new Chartest({ username: 'My Username' }).",
      );
    }
    if (password === undefined) {
      throw new Errors.ChartestError(
        "The CHARTEST_PASSWORD environment variable is missing or empty; either provide it, or instantiate the Chartest client with an password option, like new Chartest({ password: 'My Password' }).",
      );
    }
    if (apiKey === undefined) {
      throw new Errors.ChartestError(
        "The CHARTEST_API_KEY environment variable is missing or empty; either provide it, or instantiate the Chartest client with an apiKey option, like new Chartest({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('CHARTEST_LOG'), "process.env['CHARTEST_LOG']", this) ??
      defaultLogLevel;
  ...
  static Chartest = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static ChartestError = Errors.ChartestError;
...
Chartest.Todos = Todos;
Chartest.Users = Users;
Chartest.Tags = Tags;
export declare namespace Chartest {
