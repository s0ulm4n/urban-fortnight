...
```sh
npm install git+ssh://git@github.com:stainless-sdks/chartest-typescript.git
```
...
const todos = await client.todos.list().catch(async (err) => {
  if (err instanceof Chartest.APIError) {
...
1. Via the `CHARTEST_LOG` environment variable
...
const client = new Chartest({
  logger: logger.child({ name: 'Chartest' }),
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/chartest-typescript/issues) with questions, bugs, or suggestions.
